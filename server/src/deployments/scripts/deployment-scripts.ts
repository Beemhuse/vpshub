...
const sanitizeRootDir = (rootDir?: string): string => {
  if (!rootDir) return '';
  const trimmed = rootDir.trim();
  if (!trimmed) return '';
  if (trimmed.startsWith('/') || trimmed.startsWith('~')) {
    throw new Error('Root directory must be a relative path');
  }
  if (trimmed.includes('..')) {
    throw new Error('Root directory cannot contain ".."');
  }
...
export const generateDockerDeployScript = (
  projectName: string,
  deploymentId: string,
...
${generateSslScript(domain, isCustomDomain, 5)}

echo "Deployment successful!"
`;
};

export const generateStaticDeployScript = (
  projectName: string,
  deploymentId: string,
  template: Template | null,
...
export const generateNodeDeployScript = (
  projectName: string,
  deploymentId: string,
  template: Template | null,
  repositoryUrl: string,
  ...
  const rootDir = sanitizeRootDir(dto?.rootDir);
...
echo "[4/7] Starting application with PM2 on port $PORT..."
...
echo "[5/7] Cleaning old nginx configs..."
...
${generateSslScript(domain, isCustomDomain, 7)}

echo "Deployment successful!"
`;
};

export const generateSslScript = (
  domain: string,
  isCustomDomain: boolean,
  stepNum: number = 4,
): string => {
